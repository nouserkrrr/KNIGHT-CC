...
def logo():
    figlet = Figlet(font="standard").renderText("KNIGHT MODS")
    return (gn + figlet)
print (logo())
print (bl + "[-] Powered by KNIGHT MODS Team ")
print (gn + "[+] Made By @KNIGHT_MONEY")
print (cy + "[=] KNIGHT CC Tools Version : 1.1")
...
def genscard():
    ...
    return (gn + "[-] Brand : %s\n[-] Card Number : %s\n[-] Bank : %s\n[-] Name : %s\n[-] Address : %s\n[-] Country : %s\n[-] Money Range : %s\n[-] CVV : %s\n[-] Expiry : %s\n[-] Pin : %s\n============================\n[*] Telegram : @KNIGHT_MONEY" % (card['IssuingNetwork'] , card['CardNumber'] , card['Bank'] , card['Name'] , card['Address'] , card['Country'] , card['MoneyRange'] , card['CVV'] , card['Expiry'] , card['Pin']) + cv)
...
if opr == "1":
    ...
    print (mag + "[$] Telegram : @KNIGHT_MONEY" + cv)
